            ...
            <div class="row">
                        ...
                        <ul class="list-group list-group-flush">
                            <c:forEach var="lession" items="${lessions}">
                                <li class="list-group-item ${lession.lessionId == currentLession.lessionId ? 'active' : ''}">
                                    <a style="color: ${lession.lessionId == currentLession.lessionId ? '#fff' : '#333'}" href="LessonStudentController?action=learn&lessionId=${lession.lessionId}">
                                        ${lession.title}
                ...
                <div class="col-md-9">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <c:choose>
                                <c:when test="${currentLession.videoUrl != null && currentLession.videoUrl != '' && currentLession.docsLink != null}">
                                    <iframe src="${currentLession.videoUrl}" width="100%" height="600px" class="mb-3"></iframe>
                                        <c:choose>
                                            <c:when test="${currentLession.docsLink.contains('docs.google.com')}">
                                            ...
                                            <div id="file-preview">
                                                <script>
                                                    function previewFile(fileUrl) {
                                                        const supportedExtensions = ['pdf', 'jpg', 'jpeg', 'png', 'gif', 'txt', 'csv', 'html'];
                                                        ...
                                                        if (supportedExtensions.includes(fileExtension)) {
                                                            ...
                                                            document.getElementById('file-preview').innerHTML =
                                                                    '<p>Cannot preview this file. <a href="' + fileUrl + '" target="_blank" download>Download it here</a>.</p>';
                                                        }
                                                    }
                                                    previewFile('<c:out value="${currentLession.docsLink}" />');
                                                </script>
                                            </div>
                                        </c:otherwise>
                                    </c:choose>
                                </c:when>
                                <c:when test="${currentLession.videoUrl != null && currentLession.videoUrl != ''}">
                                    <c:choose>
                            ...
                            <form id="completeForm" action="LessonStudentController?action=markCompleted" method="post">
