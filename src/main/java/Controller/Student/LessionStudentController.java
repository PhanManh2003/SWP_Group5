 ...
 * @author HP
 */
@WebServlet(name = "LessionStudentController", urlPatterns = {"/LessionStudentController"})
public class LessionStudentController extends HttpServlet {
...
// Hàm lấy chi tiết bài học
    private void getLessionDetail(HttpServletRequest request, HttpServletResponse response, int studentId)
        ...
        try {
            ...
            Lession lession = lessionDao.getLessionByIdAndStudentId(lessionId, studentId); 

            if (lession != null) {
                request.setAttribute("currentLession", lession);
                List<Lession> lessions = lessionDao.getLessionsByCourseId(lession.getCourseId(), studentId);
        ...
        try {
            ...
            if (currentClass != null) {
                ...
                if (studentJoin != null) {
                    ...
                    List<Lession> lessions = lessionDao.getLessionsByCourseId(currentClass.getCourseID(), studentId);
                    request.setAttribute("lessions", lessions);
...
// Hàm đánh dấu bài học hoàn thành
    ...
    private void markLessionCompleted(HttpServletRequest request, HttpServletResponse response, int studentId)
        ...
        try {
            ...
            if (isMarked) {
                response.sendRedirect("LessionStudentController?action=learn&lessionId=" + lessionId);
            } else {
                response.sendRedirect("LessionStudentController?action=learn&lessionId=" + lessionId + "&error=Could not mark completed");
            }
        } catch (Exception e) {
            System.out.println("Error: " + e);
