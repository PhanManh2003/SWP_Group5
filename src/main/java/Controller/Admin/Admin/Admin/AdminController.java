...
public class AdminController extends HttpServlet {
    ...
    private void editAdmin(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            String oldPassString = request.getParameter("old-password");
            int idAdminNumber;
            try {
            ...
            if (!oldPassString.equals(request.getParameter("oldPassword"))) {
                response.sendRedirect("AdminController?action=edit&adminID=" + idAdmin + "&error=Old password  is not valid");
                return;
            }
    ...
    private String validatePhoneNumber(String phone) {
        // Kiểm tra độ dài (ví dụ: 10-15 số, tùy theo yêu cầu hệ thống)
